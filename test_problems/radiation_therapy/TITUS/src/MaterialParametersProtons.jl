...
struct MaterialParametersProtons
    ...
    function MaterialParametersProtons(settings::Settings,OmegaMin=5)
        ...
        @timeit to "stopping power" begin
            root = pkgdir(TITUS) 
            S_tab_PSTAR = get_S(E_tab_PSTAR,rho,comp_vector,joinpath(root, "data/proton_S_data_topas"))
        end
        E_tab_PSTAR= dropdims(E_tab_PSTAR, dims = tuple(findall(size(E_tab_PSTAR) .== 1)...)).+ E_rest
        println("Determined material parameters")

        root = pkgdir(TITUS) 
        file_path = joinpath(root, "data/proton_S_data_topas")
        _, E_tab_TOPAS, materials = read_stopping_power_file(file_path)
        E_tab_TOPAS= dropdims(E_tab_TOPAS, dims = tuple(findall(size(E_tab_TOPAS) .== 1)...)).+ E_rest
    
        new(S_tab_PSTAR,E_tab_PSTAR,E_tab_TOPAS,sigma_ce, sigma_xi,comp_vector, materials);
     end
    ...
    function get_S(E_MeV,rho,comp_vector,file_name="data/proton_S_data_topas")
        ...
        num_energies, energies, materials = read_stopping_power_file(file_path)
